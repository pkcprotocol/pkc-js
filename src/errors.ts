export enum messages {
    ...
    ERR_CAN_NOT_CREATE_A_COMMUNITY_WITH_BOTH_ANCHOR_AND_IDENTIFIER = "Can't create a community with both an anchor and an address/name/publicKey: a delegated community is keyed by its anchor, so any other identifier would be silently ignored",
    ERR_ANCHOR_PUBLIC_KEY_IS_INVALID = "anchor.publicKey is not a valid IPNS name: it becomes the community's address, so it has to be the same B58 representation community.publicKey uses",
    ERR_COMMUNITY_IS_NOT_DELEGATED = "This community has no anchor, so it publishes under its own key and has no anchor record to prepare or publish",
    ERR_UNABLE_TO_DETERMINE_ANCHOR_SEQUENCE = "Unable to determine the next anchor record sequence: neither this node nor the network knows one, and answering 0 could produce a record that can never win",
    ERR_ANCHOR_IPNS_RECORD_IS_INVALID = "The anchor IPNS record is malformed or is not signed by the community's anchor key",
    ERR_ANCHOR_IPNS_RECORD_POINTS_TO_DIFFERENT_MINTER = "The anchor IPNS record does not point at this node's minter key for this community",
    ERR_ANCHOR_IPNS_RECORD_SEQUENCE_IS_NOT_GREATER = "The anchor IPNS record's sequence is not strictly greater than the highest already accepted (replay or rollback)",
    ERR_DELEGATED_COMMUNITY_HAS_NO_ANCHOR_RECORD = "Can't start a delegated community before its anchor record has been published: nothing points the anchor at this node's minter, so the community would not be resolvable",
    ...
    ERR_CROSSPOST_CID_DOES_NOT_MATCH_EMBEDDED_COMMENT = "crosspost.cid is not the CID of the embedded crosspost.comment",
    ...
    ERR_CROSSPOST_CHAIN_EXCEEDS_MAX_DEPTH = "The crosspost chain is nested deeper than the protocol's maximum crosspost depth",
    ERR_NOT_ALLOWED_TO_PUBLISH_CROSSPOSTS = "This community does not allow crossposts",
    ERR_CROSSPOST_CHAIN_EXCEEDS_COMMUNITY_MAX_DEPTH = "The crosspost chain is nested deeper than this community's features.maxCrosspostDepth",
    ...
    ERR_CHALLENGE_EXCLUDE_ROLE_FIELD_RENAMED = "community.settings.challenges[x].exclude[y].role has been renamed to exclude.roles",
    ...
    ERR_PUBLICATION_SIGNED_TO_MINTER_INSTEAD_OF_ANCHOR = "The communityPublicKey field of publication is this delegated community's minter key; publications must be signed to the community's anchor identity",
    ...
    ERR_COMMENT_HAS_INVALID_LINK_FIELD = "This community requires every post to include a valid .link (https URL)",
