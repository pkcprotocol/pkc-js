...
describe.concurrent(`pkc.communities`, async () => {
    it(`pkc.communities updates after creating a new community`, async () => {
...
describe.concurrent(`pkc.destroy()`, async () => {
    ...
    it(`pkc.destroy() should stop updating local subs`, async () => {
        ...
        community._setUpdatingStateNoEmission = community._setUpdatingStateWithEventEmissionIfNewState = () => {
            calledUpdate = true;
        };
    ...
    itIfRpc(`pkc.destroy() should not stop running local communities (RPC client)`, async () => {
        ...
        await publishRandomPost({ communityAddress: community.address, pkc: remotePKC }); // if we can publish a post, the community is running
