...
// (found by review on PR #313). attachSubscriptionHandlersDeferred used to check
// `opts.isStale() || !subscriptionActive(id)` ONCE, before draining the pending buffer, and
// emitAllPendingMessages then looped over every buffered notification with no re-check between
// messages. If a replayed handler called stop(), the stop's synchronous prefix cleared the
// subscription id immediately (src/community/rpc-remote-community.ts, stop()), yet the client
// only deletes _subscriptionEvents[id] after the awaited wire unsubscribe resolves, so the
// forEach kept going and delivered the REMAINING buffered notifications into an instance that
// was already stopping. This is the per-message sibling of the timer-level stop race covered by
...
// nothing is delivered afterwards, neither to the community listener nor to pkc. Before the fix
// (the replay now drains through _emitPendingMessagesWhile, re-checking ownership before each
// message) this test was RED because the loop delivered the second buffered error after stop().
//
...
describe("RPC: buffered replay stops delivering once a replayed handler calls stop()", () => {
    ...
    beforeAll(async () => {
        dataPath = uniqueTmpDataPath("pkc-rpc-stop-during-replay-test");
    ...
    itIfRpc("calling stop() from the error handler drops the remaining buffered notifications", async () => {
        ...
        try {
            ...
            await pollUntil(() => communityLevelOrdinals.includes(1), { timeoutMs: 5_000, intervalMs: 100 });
