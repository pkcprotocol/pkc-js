// Regression test for issue #314: over PKC RPC, RpcLocalCommunity.start() used to attach its
// notification handlers and synchronously call emitAllPendingMessages(subscriptionId) INSIDE the
// awaited start() call (src/community/rpc-local-community.ts), replaying every buffered
// notification before the caller of start() got a chance to attach an "error" listener. Any
// error the server emitted at start-subscribe time (before the JSON-RPC startCommunity response)
// then bubbled to pkc via the listenerCount("error") === 1 rule in remote-community.ts, so
// callers following the natural `await community.start(); community.on("error", ...)` order
// never saw it and pkc emitted it as unhandled instead. The race was first fixed for
// RpcRemoteCommunity.update() only (macrotask-deferred replay in _initRpcUpdateSubscription);
// start() got the same fix afterwards (#314), which this test pins.
//
...
// buffer, and used to be replayed synchronously inside community.start() (the deferred replay
// now delivers it on a later tick).
//
...
describe("RPC: community error emitted at start-subscribe time reaches a listener attached after start() (#314)", () => {
    ...
    beforeAll(async () => {
        dataPath = uniqueTmpDataPath("pkc-rpc-start-subscribe-time-error-test");
