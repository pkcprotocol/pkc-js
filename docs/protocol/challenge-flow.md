...
One matcher is built per challenge request (`ParsedChallengeRequest.authorIdentityMatcher`) and threaded through validation, the excludes, the challenges and storage, so a publication's domain is resolved at most once instead of once per call site. A key-derived entry is compared before any domain is resolved, so a moderator listed under both their public key and their domain matches for free and stays authorized while the resolver is down. See issue #354.
