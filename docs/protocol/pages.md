...
6. **Single-chunk shortcut:** when every preloaded sort fits in a single page and the embedded pages hold the whole comment set, only those pages are published and the other sorts are not generated at all; clients re-sort the embedded comments locally (see "Client-side re-sorting" in `page-sorts.md`). Skipping the other sorts is only sound when a client can rebuild them from what is embedded, so a preloaded sort whose window or filter dropped comments (a preloaded `topDay` on a board with older posts) takes the full path next to other sorts: its page embeds, every other sort goes to `pageCids`. A custom sort that is not preloaded is therefore unavailable on a board that fits in one unfiltered preloaded page (the reason `active-page-sort` style boards preload their custom sort). With no preloaded sort there is no shortcut and every sort is in `pageCids` with `pages: {}`
...
8. **How an entry is built.** A page entry is the comment plus the CommentUpdate the community last signed, nested reply pages included: `replies` is a signed field, so the entry must carry it byte for byte. The community stores only CID refs for a comment's reply pages (the `replies` column, which `stale_replies` and unpinning read) and rebuilds each post's tree from the reply rows when a page needs it: sorting works on the lean post rows, entries are serialized a batch of posts at a time (one indexed read of their subtrees, `queryRepliesUnderPosts`), and a page fetches the entries it does not hold when it is built, so a board's reply trees stream through generation and are never held at once. Serialized entries are kept across generations while their CommentUpdate's `updatedAt` is unchanged, under a byte budget derived from the heap limit, so a steady board re-serializes only the posts that changed. Pages are canonical JSON (keys sorted, what `safe-stable-stringify` gives), built with the native stringifier from rows the database mapper already lists in sorted order
