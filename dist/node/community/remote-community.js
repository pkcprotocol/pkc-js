import { createAbortError, doesDomainAddressHaveCapitalLetter, hideClassPrivateProps, ipnsNameToIpnsOverPubsubTopic, isIpns, isStringDomain, pubsubTopicToDhtKey, shortifyAddress, timestamp } from "../util.js";
...
import { findUpdatingCommunity, refreshTrackedCommunityAliases, trackUpdatingCommunity, untrackUpdatingCommunity } from "../pkc/tracked-instance-registry-util.js";
export class RemoteCommunity extends TypedEmitter {
    constructor(pkc) {
        super();
        // to be overridden by local community classes
        this.startedState = "stopped";
        this.raw = {};
        this._updatingCommunityInstanceWithListeners = undefined; // The pkc._updatingCommunities we're subscribed to
    ...
    _updateLocalPostsInstance(newPosts) {
        ...
        if (!newPosts)
            ...
            if (typeof postsPagesCreationTimestamp !== "number")
                throw Error("community.updatedAt should be defined when updating posts");
    ...
    initCommunityIpfsPropsNoMerge(newProps) {
    ...
    _updateIpnsPubsubPropsIfNeeded(newProps) {
        ...
        if (!this.pubsubTopicRoutingCid) {
            if ("pubsubTopicRoutingCid" in newProps)
                ...
                this.pubsubTopicRoutingCid = pubsubTopicToDhtKey(newProps.pubsubTopic ||
                    this.pubsubTopic ||
                    ("address" in newProps ? newProps.address : undefined) ||
                    this.address);
        }
    }
    initRemoteCommunityPropsNoMerge(newProps) {
        ...
        const explicitPublicKey = "publicKey" in newProps ? newProps.publicKey : undefined;
        ...
        if ("updateCid" in newProps && newProps.updateCid)
    ...
    _isRetriableErrorWhenLoading(err) {
        ...
        if (err.code === "ERR_COMMUNITY_SIGNATURE_IS_INVALID" ||
        ...
        return {
            ...
            update: () => {
                if (!communityInstance.raw.communityIpfs || !communityInstance.updateCid) {
                    if (communityInstance.publicKey)
                        this._clearDataForKeyMigration(communityInstance.publicKey);
                }
                else {
                    this.initCommunityIpfsPropsNoMerge(communityInstance.raw.communityIpfs);
                    this.updateCid = communityInstance.updateCid;
                }
                if (typeof communityInstance.nameResolved === "boolean")
                    this.nameResolved = communityInstance.nameResolved;
                log(`Remote Community instance`, this.address, `received update event from pkc._updatingCommunities[${this.address}] with updatedAt`, this.updatedAt, "that's", timestamp() - this.updatedAt, "seconds old");
                this.emit("update", this);
            },
            error: (error) => {
        ...
        for (const clientType of clientKeys)
            if (this.clients[clientType])
                for (const clientUrl of Object.keys(this.clients[clientType]))
                    this.clients[clientType][clientUrl].mirror(this._updatingCommunityInstanceWithListeners.community.clients[clientType][clientUrl]);
    ...
    async _cleanUpUpdatingCommunityInstanceWithListeners() {
        ...
        if (typeof updatingCommunity.ipnsName === "string")
            this._ipnsName = updatingCommunity.ipnsName;
        if (typeof updatingCommunity.ipnsPubsubTopic === "string")
            this._ipnsPubsubTopic = updatingCommunity.ipnsPubsubTopic;
        ...
        this._updatingCommunityInstanceWithListeners.community.removeListener("statechange", this._updatingCommunityInstanceWithListeners.statechange);
        ...
        this._updatingCommunityInstanceWithListeners.community._numOfListenersForUpdatingInstance--;
        if (this._updatingCommunityInstanceWithListeners.community._numOfListenersForUpdatingInstance === 0 &&
    ...
    async stop() {
        ...
        if (this.state !== "updating")
            throw new PKCError("ERR_CALLED_COMMUNITY_STOP_WITHOUT_UPDATE", { address: this.address });
        ...
        if (this._updatingCommunityInstanceWithListeners)
            await this._cleanUpUpdatingCommunityInstanceWithListeners();
        else {
...
//# sourceMappingURL=remote-community.js.map